...
- 自動 E2E: `tests/e2e/reduce-motion.spec.ts`。`/` ランディング上で `.marquee` / `.sparkle` / `.cutie-float` と `.btn-puff` の computed style を読み、検証 1〜3 を踏む
- 将来 OS 連動を再導入する際は、上記に加え `page.emulateMedia({ reducedMotion: 'reduce' })` ベースのケースを追加する
