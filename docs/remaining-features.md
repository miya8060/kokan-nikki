...
要件 ID の意図のうち一部しか実装で表現されていない ID。各 ID の「なぜ MVP では現状維持にしているか」の判断ログは `docs/requirements.md` §10「MVP 運用解釈」に集約しているので、要件側の温度感はそちらを一次ソースにする。
...
testing.md §5 が要求するレイヤをすべて満たしており、緊急度の高い「テスト不足」候補は残っていない。**MVP 内のテスト整備としては done**。残るタスクはリリース運用手順 (Vercel env / Neon / Resend ドメイン検証 / 本番スモーク / Cron 動作確認) に移行する。

軽くカバーが薄いが MVP scope 外と整理済の領域:
